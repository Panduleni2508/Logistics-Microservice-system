...
// Run the main menu and handle the options
...
// Get the user's selected option from the menu
function getMenuOption() returns int|error {
    return int:fromString(io:readln("Enter your choice (1-3): "));
}

// Handle the user's selection based on the menu option
...
// Get the shipment type from the user
...
// Display a confirmation after submitting a request
...
// Track a shipment using a tracking number
...
// Display confirmation for tracking request submission
...
// Send a message to a Kafka topic (reuse for multiple topics)
